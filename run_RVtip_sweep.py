...
"""
run_RVtip_sweep.py
R(Vtip) sweep runner that uses core_fem.py (numerically identical to fitting).
"""
...
def main(argv=None):
    ...
    ap.add_argument("--Vtip_list", type=float, nargs=3, default=[-6.0, -1.0, 30], help="start stop num for Vtip [V]")
    ap.add_argument("--s_fixed", type=float, default=8.0, help="fixed tip–sample distance s [nm]")
    ...
    ap.add_argument("--out_root", type=str, default="RV_out", help="root dir; a timestamped run dir will be created under it")
    ...
    with make_stdout_tee(run_dir):
        ...
        geom = GeometricParameters(l_sio2=args.l_sio2, tip_radius=float(args.tip_radius), tip_height=float(args.s_fixed))
        ...
        with open(out_csv, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["Vtip (V)"]+[f"R_{lev:.3f}V" for lev in LEVELS])
            for i, V in enumerate(Vtips):
                w.writerow([V] + [R_dict[float(lev)][i] for lev in LEVELS])
        print("[INFO] saved:", out_csv)

        for lev in LEVELS:
            levf = float(lev)
            p = Path(run_dir)/f"{stamp}_R_vs_Vtip_{levf:.3f}V.csv"
        ...
        ax.set_xlabel("Vtip [V]"); ax.set_ylabel("R [nm]")
